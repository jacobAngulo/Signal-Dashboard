...
def signals(producer: Annotated[list[str] | None, Query()] = None,
    ...
    producers = [producer] if isinstance(producer, str) else list(producer or [])
    unknown = [p for p in producers if p not in STORE.producers]
    if unknown:
        raise HTTPException(404, f"unknown producer {unknown[0]}")
    rows = enriched_decisions(producers or None, date_from, date_to, ticker,
