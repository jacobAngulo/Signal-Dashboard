...
class DashboardApiUxTests(unittest.TestCase):
    def test_score_search_treats_user_text_as_literal(self):
        producer = SimpleNamespace(
            scores={"2026-07-20": pd.DataFrame([
                {"ticker": "A[B", "signal_score": 0.3},
                {"ticker": "ABC", "signal_score": 0.2},
            ])},
            spec={"history_metric": "signal_score"},
            dates=["2026-07-20"],
        )
        store = SimpleNamespace(producers={"foundry": producer})
        with patch.object(main, "STORE", store):
            result = main.scores(
                "foundry", "2026-07-20", sort=None, dir="desc",
                limit=10, offset=0, q="[",
            )
    ...
    def test_score_only_ticker_has_a_valid_detail_page(self):
        ...
        store = SimpleNamespace(
            producers={"foundry": producer}, prices={},
    ...
    def test_unresolved_ticker_keeps_descriptive_insights_available(self):
        ...
        signals = [
            {
                "producer": "foundry", "date": "2026-07-18", "decision": "BUY",
        ...
        result = main._ticker_insights(
            signals,
            series,
            {"foundry": [{"date": "2026-07-19", "metric": 0.31}]},
        )

        self.assertEqual(result["status"], "available")
        self.assertTrue(result["has_action_warning"])
        self.assertEqual(result["buy_count"], 1)
        self.assertEqual(result["latest_scores"]["foundry"]["metric"], 0.31)
    ...
    def test_intraday_chart_is_separate_from_daily_performance_book(self):
        ...
        signals = [{
            "id": "s1", "producer": "foundry", "date": "2026-07-27",
