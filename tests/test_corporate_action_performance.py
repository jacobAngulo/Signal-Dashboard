...
class CorporateActionPerformanceTests(unittest.TestCase):
    ...
    def test_enrich_preserves_signal_price_but_uses_gateway_entry_and_exit(self):
        ...
        with patch("backend.metrics.STORE", Store()):
            row = enrich(
                {
                    "id": "x", "producer": "daily", "date": "2026-06-25",
    ...
    def test_enrich_does_not_overflag_observed_action_before_signal(self):
        ...
        with patch("backend.metrics.STORE", store):
            row = enrich({
                "id": "flagged", "producer": "daily", "date": "2026-06-26",
    ...
    def test_analytics_counts_but_excludes_unresolved_buy_everywhere(self):
        class Producer:
            ...
            def run_rows(self):
                return [{"producer": "daily", "date": "2026-06-25", "n_buy": 1, "status": "ok"}]
        ...
        class Store:
            ...
            all_decisions = [
                {
                    "id": "bad", "producer": "daily", "date": "2026-06-25",
                    "ticker": "AAA", "decision": "BUY", "metric": 0.8,
                    "signal_price": 105.0,
                }
            ]
            producers = {"daily": Producer()}
        ...
        with patch("backend.metrics.STORE", Store()):
            result = analytics()
        producer = result["by_producer"]["daily"]
    ...
    def test_analytics_keeps_each_safe_horizon_on_a_flagged_signal(self):
        ...
        row = {
            "id": "partial", "producer": "daily", "date": "2026-06-25",
            "ticker": "AAA", "decision": "BUY", "metric": 0.8,
        ...
        store = type("Store", (), {"producers": {"daily": Producer()}})()
        with (
