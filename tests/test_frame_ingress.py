...
visible form of that: a producer WATCH row for BAND carried a NaN
...
class FrameIngressTests(unittest.TestCase):
    ...
    def test_ticker_page_survives_a_missing_timestamp(self):
        ...
        signals = [
            ...
            {"date": "2026-07-24", "producer": "foundry", "decision": "WATCH",
             "created_at": None},
        ]
        ...
        # One producer publishes now, so the distinct-producer count is 1. The
        # subject of this test is the NaN timestamp, not the count.
        self.assertEqual(insights["producer_count"], 1)

    def test_every_sortable_column_survives_mixed_types(self):
        """The signals feed sorts on a user-supplied column."""
        rows = [
            {"id": "a", "producer": "foundry", "date": "2026-07-29",
            ...
            {"id": "b", "producer": "foundry", "date": 20260729,
